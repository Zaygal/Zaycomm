...
type RouteCandidate = { status: 'probation' | 'validated'; lastAdvertisedAt: number; expiresAt: number };
...
class RoutingTable {
  ...
  demote(destinationHint: Uint8Array, neighborId: string): void {
    const route = this.routes.get(bytesToHex(destinationHint))?.get(neighborId);
    if (!route) return;
    route.status = 'probation'; route.expiresAt = 0;
  }
  ...
  lookup(destinationHint: Uint8Array, trustScores: Map<string, number>, nowMs: number = Date.now()): string | null {
    ...
    for (const [neighborId, route] of candidates) {
      ...
      if (route.status === 'validated') { if (score > bestValidatedScore) { bestValidatedScore = score; bestValidated = neighborId; } }
      else if (score > bestProbationScore) { bestProbationScore = score; bestProbation = neighborId; }
    }
    return bestValidated ?? bestProbation;
  }
  hasRoute(destinationHint: Uint8Array, nowMs: number = Date.now()): boolean { return this.lookup(destinationHint, new Map(), nowMs) !== null; }
}
...
function encodeAuthenticatedSync(identity: Identity, inner: SyncPayloadTuple): Uint8Array {
  ...
  return Uint8Array.from(cbor.encode([identity.publicKey, signature, innerBytes] as AuthenticatedSyncTuple));
}
...
export class RelayNode {
  ...
  receiveAdvertisement(fromNeighborId: string, ad: RoutingAdvertisement): void {
    ...
    this.seenRoutingAdvertisements.set(replayKey, now); this.routingTable.learnFromAdvertisement(fromNeighborId, ad, now);
  ...
  private attemptQueuedDelivery(destinationHint: Uint8Array): void {
    this.purgeStaleRoutingState(); const nextHopId = this.routingTable.lookup(destinationHint, this.neighborTrust); if (!nextHopId) return;
  ...
  initiateSync(neighborId: string): boolean {
    ...
    return this.sendEnvelopeOverTransport(neighborId, createSyncEnvelope(encodeAuthenticatedSync(this.identity, [0, entries])));
  }
  ...
  private handleSyncPacket(envelope: Envelope, fromNodeId: string | null): DeliveryResult {
    ...
    if (tuple[0] === 0) {
      ...
      if (missingIds.length > 0) this.sendEnvelopeOverTransport(fromNodeId, createSyncEnvelope(encodeAuthenticatedSync(this.identity, [1, missingIds])));
    ...
    if (tuple[0] === 1) {
      ...
      this.sendEnvelopeOverTransport(fromNodeId, createSyncEnvelope(encodeAuthenticatedSync(this.identity, [2, wireEnvelopes])));
      return { outcome: 'delivered', envelope };
    }
  ...
  receiveEnvelope(envelope: Envelope, fromNodeId: string | null): DeliveryResult {
    ...
    if (bytesEqual(envelope.header.destinationHint, this.ownDestinationHint)) {
      if (envelope.header.packetType === PacketType.Ack) {
        ...
        this.routingTable.markValidated(pending.destinationHint, pending.neighborId); this.neighborTrust.set(pending.neighborId, (this.neighborTrust.get(pending.neighborId) ?? 0) + 1); this.pendingAcks.delete(ackedIdHex);
