...
class ZaycommNotificationModule(private val context: ReactApplicationContext) : ReactContextBaseJavaModule(context) {
    ...
    fun createChannels(promise: Promise) {
        if (Build.VERSION.SDK_INT >= 26) {
            val m = manager()
            m.createNotificationChannel(NotificationChannel(CHANNEL_MESSAGES, "Messages", NotificationManager.IMPORTANCE_HIGH).apply {
                description = "New Zaycomm messages"
            })
            m.createNotificationChannel(NotificationChannel(CHANNEL_CONNECTIONS, "Connections", NotificationManager.IMPORTANCE_DEFAULT).apply {
                description = "Nearby node and secure link events"
            })
            m.createNotificationChannel(NotificationChannel(CHANNEL_SECURITY, "Security", NotificationManager.IMPORTANCE_HIGH).apply {
                description = "Zaycomm security events"
            })
        }
    ...
    fun showNotification(id: Int, channel: String, title: String, body: String, promise: Promise) {
        try {
            createChannels(Promise.resolvePromise())
            ...
            promise.reject("NOTIFICATION_ERROR", e.message ?: "Unable to show notification")
        }
    }
    ...
    private object Promise {
        fun resolvePromise(): com.facebook.react.bridge.Promise = object : com.facebook.react.bridge.Promise {
