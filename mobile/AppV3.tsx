...
import{Alert,DeviceEventEmitter,NativeEventEmitter,NativeModules,PermissionsAndroid,Platform,Pressable,StyleSheet,Text,View}from'react-native';
...
function validIdentity(payload:string):Identity{
 ...
 if(publicKey.slice(0,16)!==nodeId)throw new Error('PEER_NODE_ID_MISMATCH');
 return{nodeId,publicKey};
}

export default function AppV3(){
 const[peer,setPeer]=useState<Peer|null>(null),[linked,setLinked]=useState(false),[dismissed,setDismissed]=useState<string[]>([]),[step,setStep]=useState<'prompt'|'scanning'|'verified'>('prompt'),[identity,setIdentity]=useState<Identity|null>(null),[error,setError]=useState('');
 useEffect(()=>{AsyncStorage.getItem(DISMISSED_KEY).then(v=>{try{setDismissed(v?JSON.parse(v):[])}catch{setDismissed([])}})},[]);
 useEffect(()=>{if(!bleEvents)return;const ad=bleEvents.addListener('ZaycommBleAdvertisement',(p:Peer)=>setPeer(p));const link=bleEvents.addListener('ZaycommBleConnectionChanged',(e:{address:string;connected:boolean})=>{if(e.connected){setLinked(true)}else{setLinked(false);setPeer(current=>current?.address===e.address?null:current)}});return()=>{ad.remove();link.remove()}},[]);
 ...
 const visible=!!peer&&linked&&!dismissed.includes(peer.address);
 useEffect(()=>{if(visible){setStep('prompt');setIdentity(null);setError('')}},[visible]);
 const dismiss=async()=>{if(!peer)return;const next=[...new Set([...dismissed,peer.address])];setDismissed(next);await AsyncStorage.setItem(DISMISSED_KEY,JSON.stringify(next));await scanner?.release?.();setPeer(null);setLinked(false)};
