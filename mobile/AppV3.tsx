...
export default function AppV3(){
 const[peer,setPeer]=useState<Peer|null>(null),[linked,setLinked]=useState(false),[dismissed,setDismissed]=useState<string[]>([]),[trusted,setTrusted]=useState<string[]>([]),[step,setStep]=useState<'prompt'|'scanning'|'verified'>('prompt'),[identity,setIdentity]=useState<Identity|null>(null),[error,setError]=useState('');
 useEffect(()=>{Promise.all([AsyncStorage.getItem(DISMISSED_KEY),AsyncStorage.getItem(TRUSTED_KEY)]).then(([d,t])=>{try{setDismissed(d?JSON.parse(d):[])}catch{setDismissed([])}try{setTrusted(t?JSON.parse(t):[])}catch{setTrusted([])}})},[]);
 ...
 const visible=!!peer&&linked&&!dismissed.includes(peer.address)&&!trusted.includes(identity?.nodeId||'');
 ...
 const trust=async()=>{if(!identity)return;try{const current=JSON.parse((await AsyncStorage.getItem(TRUSTED_KEY))||'[]');const next=[...new Set([...current,identity.nodeId])];await AsyncStorage.setItem(TRUSTED_KEY,JSON.stringify(next));setTrusted(next);await scanner?.release?.();setDismissed(d=>d.filter(x=>x!==peer?.address));await AsyncStorage.setItem(DISMISSED_KEY,JSON.stringify(dismissed.filter(x=>x!==peer?.address)));setStep('prompt');setIdentity(null);setError('');}catch(e){setError(`Unable to save trust: ${String(e)}`)}};
