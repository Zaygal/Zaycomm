...
describe('C16 cross-phase adversarial campaign', () => {
  ...
  it('C6 + C8: fragment exhaustion cannot be used to allocate unbounded sync state', () => {
    ...
    for (let i = 0; i < 64; i++) {
      reassembler.addFragment({ messageId: new Uint8Array([i & 0xff, (i >> 8) & 0xff]), index: 0, total: 2, data: new Uint8Array(32) }, peer);
    }
  ...
  it('C7 + C9: replayed advertisement provenance cannot become a different route', () => {
    ...
    const original = createRouteProvenance(destination.publicKey, hint(1), advertiser.publicKey, 'relay-a', 100);
    const replayedThroughDifferentNeighbor = createRouteProvenance(destination.publicKey, hint(1), advertiser.publicKey, 'relay-b', 100);
    expect(matchesRouteTrustBinding(bindRouteTrust(original, 10), replayedThroughDifferentNeighbor)).toBe(false);
  });
  ...
  it('C5 + C6: authenticated fragmentation still obeys bounded reassembly', () => {
    ...
    const peer = 'authenticated-peer';
    for (let i = 0; i < 32; i++) {
      reassembler.addFragment({ messageId: new Uint8Array([0xaa, i]), index: 0, total: 2, data: new Uint8Array(16) }, peer);
    }
    expect(reassembler.pendingCount()).toBeLessThanOrEqual(16);
  });
  ...
  it('C9 + C11: route abuse does not grant unlimited broadcast creation', () => {
    ...
    for (let i = 0; i < 20; i++) createBroadcastMessage(origin, payload);
